...
from keyboards import (
    ...
    settings_info_text, back_keyboard, referral_inline_keyboard, partners_keyboard
)
from states import RegisterState, SettingsState, ReferralState, PartnersState, SupportState
...
router = Router()
REFERRAL_CARD_BUTTONS = ["💳 Картани қўшиш/янгилаш", "💳 Добавить/обновить карту"]
REFERRAL_WITHDRAW_BUTTONS = ["💰 Бонусни ечиб олиш", "💰 Вывести бонус"]
REFERRAL_HOME_BUTTONS = ["🏠 Бош менью", "🏠 Главное меню"]
...
def _order_status_label(status: str, lang: str) -> str:
    ...
    if st == "cancelled":
        return "Бекор қилинган" if lang == "uz" else "Отменено"
...
async def cmd_start(message: Message, state: FSMContext, bot: Bot):
    ...
    if user_id in ADMIN_IDS:
        ...
        if user and user.get("registered"):
            ...
            await message.answer("👨‍💼 Хуш келибсиз, Admin!", reply_markup=main_menu_keyboard(lang))
            return
...
async def menu_rates(message: Message, bot: Bot):
    ...
    from exchange_config import CURRENCIES, get_currency_by_id
    db     = load_db()
    manual = db.get("manual_rates", {})

    if not manual:
        text = "⏳ Курслар ҳали киритилмаган." if lang == "uz" else "⏳ Курсы ещё не введены."
    ...
    for key, info in manual.items():
        ...
        if len(parts) != 2:
            continue
        from_cur = get_currency_by_id(parts[0])
        to_cur   = get_currency_by_id(parts[1])
        if not from_cur or not to_cur:
            continue
        rate_v = info.get("rate", 0)
        if rate_v and rate_v < 1:
            rate_disp = f"1 {to_cur['name']} = {round(1 / rate_v):,} {from_cur['name']}"
        else:
            rate_disp = f"1 {from_cur['name']} = {rate_v:,} {to_cur['name']}"
        lines.append(f"🔷 {from_cur['name']} ➡️ {to_cur['name']}: {rate_disp}")

    last_upd = db.get("last_rate_update", "—")
    text = ("📊 Жорий курслар:\n\n" if lang == "uz" else "📊 Текущие курсы:\n\n")
    text += "\n".join(lines)
    text += f"\n\n🕐 {'Янгиланган' if lang == 'uz' else 'Обновлено'}: {last_upd}"
    await message.answer(text)



@router.message(F.text.in_(["👥 Ҳамёнлар", "👥 Партнёры"]))
...
@router.message(F.text.in_(["👥 Реферал", "👥 Реферал"]))
...
async def referral_card_save(message: Message, state: FSMContext, bot: Bot):
    if (message.text or "").strip() in REFERRAL_HOME_BUTTONS:
        ...
        await message.answer(t(lang, "main_menu"), reply_markup=main_menu_keyboard(lang))
        return
    if (message.text or "").strip() in ["🔙 Орқага", "🔙 Назад"]:
        await state.clear()
        await send_referral_panel(message, bot)
        return
...
@router.message(F.text.in_(["📞 Қайта алоқа", "📞 Обратная связь"]))
...
@router.message(F.text.in_(["🔙 Орқага", "🔙 Назад"]))
